...
| Correlazione sessione↔pacchetto imprecisa senza hook `SessionStart` installato | Fallback euristico "ultimo pacchetto attivato per quel `cwd`" da `activation_log` |
...
| **[CRITICAL, deferred da Fase 1]** Nessuna protezione CORS/Origin sugli endpoint mutanti (`POST`/`DELETE /mcp/servers`, `POST /projects`, `POST /system/shutdown`): una pagina web malevola aperta nello stesso browser può inviare richieste cross-origin "simple" (no preflight) che scrivono config MCP eseguibili da Claude Code o spengono il processo | Da risolvere prima che l'app sia usata oltre il contesto di sviluppo locale: middleware di validazione header `Origin` sulle richieste mutanti (confronto con host/porta della richiesta stessa), eventualmente + validazione che `project_path` sia un progetto noto prima di scrivere |
| **[HIGH, deferred da Fase 1]** `json_store.write_json` non è atomica (`write_text` tronca in place): un crash o due richieste concorrenti a metà scrittura possono corrompere file reali di Claude Code (`~/.claude.json`, `.mcp.json`) | Scrivere su file temporaneo nella stessa directory + `os.replace()` per la sostituzione atomica |
